...
*   **Modularización CSS (`@import`):** La hoja de estilos principal (`main.css`) sirve exclusivamente como punto de entrada (índice) que importa secuencialmente módulos específicos (`variables.css`, `reset.css`, `layout.css`, `components.css`, `lightbox.css`, `utilities.css`). Esto mejora radicalmente la mantenibilidad al separar responsabilidades, sin requerir alterar las referencias en el documento HTML original.
...
*   **Accesibilidad (A11y) y Gestión de Foco:** Se garantiza la navegación fluida por teclado (Screen Readers). Las tarjetas interactivas de las galerías (`.photo-item`) cuentan con `tabindex="0"` programático y responden a las teclas `Enter` y `Espacio`. Al salir de un modal (como el `<dialog>` del Lightbox), un escuchador de eventos `close` restaura instantáneamente el foco al último elemento utilizado, conservando el contexto del usuario en la pantalla.
...
*   **Lógica DRY (Don't Repeat Yourself):** Se implementan funciones de envoltura auxiliares (`readJsonFile` y `writeJsonFile`) para encapsular por completo toda la lógica de lectura asíncrona, parseo y serialización de los archivos JSON del sistema de archivos, lo que reduce la redundancia de bloques `try...catch` en los controladores de las rutas de Express y hace el backend altamente escalable.
...
*   **Reordenación Drag & Drop Inteligente:** Tanto las galerías en la barra lateral como las imágenes dentro de cada galería son reordenables mediante drag & drop con SortableJS. Al soltar, el orden se guarda asíncronamente en los archivos JSON. Al manipular arreglos híbridos (como Favoritos que mezclan strings planos y objetos para metadatos como `featured`), el frontend mapea e identifica cada ítem para **preservar toda la metadata oculta original** tras la nueva ordenación del DOM.
